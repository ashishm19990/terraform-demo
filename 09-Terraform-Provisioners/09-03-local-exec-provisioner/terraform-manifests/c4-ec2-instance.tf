...
resource "aws_instance" "terraform_ec2_instance" {
  ...
  tags = {
    "Name" = "terraform-ec2-instance-${terraform.workspace}-0"
  }

  # local-exec provisioner (Creation-Time Provisioner - Triggered during Create Resource)
  provisioner "local-exec" {
    command     = "echo Local-time provisioner Instanace Created at `date` ${aws_instance.terraform_ec2_instance.private_ip} >> creation-time-private-ip.txt"
